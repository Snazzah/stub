...
export default function Clicks({ data, isValidating }: { data: StatsProps; isValidating: boolean }) {
  return (
      ...
      <div className="mb-5 text-left">
        <div className="flex space-x-1 items-end">
          {isValidating ? (
            ...
            <h1 className="text-3xl sm:text-4xl font-bold">{nFormatter(data.totalClicks)}</h1>
          )}
          <Chart className="text-gray-600 w-6 h-6 mb-1" />
        </div>
        <p className="uppercase text-gray-600 text-sm font-medium">Total Clicks</p>
      </div>
      {/* @ts-ignore */}
