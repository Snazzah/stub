...
import { LocationTabs, processLocationData, StatsProps } from '@/lib/stats';
...
export default function Locations({ data: rawData }: { data: StatsProps }) {
  ...
  return (
    ...
    >
      ...
      <div className={data.locationData && data.locationData.length > 0 ? 'grid gap-4' : 'h-[300px] flex justify-center items-center'}>
        {data.locationData ? (
          data.locationData.length > 0 ? (
              ...
              <div key={idx} className="flex justify-between items-center">
                <div className="relative flex items-center z-10 w-full max-w-[calc(100%-3rem)]">
                  <span className="flex space-x-2 px-2 items-center z-10">
                    <img src={`https://flag.vercel.app/m/${code}.svg`} className="w-5 h-3" />
                  ...
                  <motion.div
                    ...
                    className="bg-orange-100 absolute h-8 origin-left rounded"
            ...
            <div className="flex flex-col gap-4 items-center">
              <ChartIllustration className="text-orange-100 w-36 h-36 opacity-50" />
