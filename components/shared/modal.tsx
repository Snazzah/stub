      ...
      {showModal && (
        <>
          <motion.div
            ref={mobileModalRef}
            key="mobile-modal"
            className="group fixed bottom-0 inset-x-0 z-50 w-screen sm:hidden cursor-grab active:cursor-grabbing"
            initial={{ y: '100%' }}
            animate={controls}
            exit={{ y: '100%' }}
            transition={transitionProps}
          ...
          >
            <div className={`h-7 ${bgColor} w-full flex items-center justify-center rounded-t-4xl border-t border-gray-200 -mb-1`}>
          ...
          <motion.div
            ref={desktopModalRef}
            key="desktop-modal"
            className="fixed inset-0 z-50 min-h-screen hidden sm:flex items-center justify-center"
            initial={{ scale: 0.95 }}
            animate={{ scale: 1 }}
            exit={{ scale: 0.95 }}
            onClick={(e) => {
