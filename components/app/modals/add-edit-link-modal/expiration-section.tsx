...
export default function ExpirationSection({ data, setData }: { data: LinkProps; setData: Dispatch<SetStateAction<LinkProps>> }) {
  const { expiresAt } = data;
  return (
    <motion.div key="expire" {...AnimationSettings}>
      <div>
        <label htmlFor="expiresAt" className="my-2 block text-sm text-gray-500">
          Automatically expires your link at a given date and time. Your link will be disabled but the data will still be kept.
        </label>
