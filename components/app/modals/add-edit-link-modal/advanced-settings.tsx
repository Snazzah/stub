...
export default function AdvancedSettings({ data, setData }: { data: LinkProps; setData: Dispatch<SetStateAction<LinkProps>> }) {
  ...
  return (
    <div>
      <div className="px-4 sm:px-16">
        <button type="button" className="flex items-center space-x-2" onClick={() => setExpanded(!expanded)}>
      ...
      {expanded && (
        <motion.div key="accordion-root" {...AnimationSettings}>
          ...
          >
            {/* UTM Builder Section */}
            <AccordionPrimitive.Item value="utm" className="rounded-t-lg border border-gray-200">
              <AccordionPrimitive.Header>
                <AccordionPrimitive.Trigger className="group flex w-full items-center justify-between space-x-2 rounded-t-lg p-5 text-left transition-all duration-75 hover:bg-gray-50 focus:outline-black active:bg-gray-100">
                  <div className="flex h-6 items-center justify-start space-x-2">
                    ...
                    <span className="text-sm font-medium text-gray-900">UTM Builder</span>
                  </div>
                  {utm_source && utm_medium && utm_campaign && <CheckCircleFill className="h-6 w-6 text-black" />}
            ...
            {/* OG Tags Section */}
            <AccordionPrimitive.Item value="og" className="border border-t-0 border-gray-200">
              <AccordionPrimitive.Header>
                <AccordionPrimitive.Trigger className="group flex w-full items-center justify-between space-x-2 p-5 text-left transition-all duration-75 hover:bg-gray-50 focus:outline-black active:bg-gray-100">
                  <div className="flex h-6 items-center justify-start space-x-2">
                    <ChevronRight className="h-5 w-5 shrink-0 text-gray-700 transition-all ease-in-out group-radix-state-open:rotate-90" />
                    <span className="text-sm font-medium text-gray-900">Custom OG Tags</span>
                  </div>
                  {title && description && image && <CheckCircleFill className="h-6 w-6 text-black" />}
                </AccordionPrimitive.Trigger>
              </AccordionPrimitive.Header>
              <AccordionPrimitive.Content className="pb-3">
            ...
            {/* Password Protection */}
            <AccordionPrimitive.Item value="password" className="border border-t-0 border-gray-200">
              <AccordionPrimitive.Header>
                <AccordionPrimitive.Trigger className="group flex w-full items-center justify-between space-x-2 p-5 text-left transition-all duration-75 hover:bg-gray-50 focus:outline-black active:bg-gray-100">
                  <div className="flex h-6 items-center justify-start space-x-2">
                    <ChevronRight className="h-5 w-5 shrink-0 text-gray-700 transition-all ease-in-out group-radix-state-open:rotate-90" />
                  ...
                  {password && <CheckCircleFill className="h-6 w-6 text-black" />}
                </AccordionPrimitive.Trigger>
              </AccordionPrimitive.Header>
            ...
            {/* Expiration Date */}
            <AccordionPrimitive.Item value="expire" className="overflow-hidden rounded-b-lg border border-t-0 border-gray-200">
              <AccordionPrimitive.Header>
                <AccordionPrimitive.Trigger className="group flex w-full items-center justify-between space-x-2 p-5 text-left transition-all duration-75 hover:bg-gray-50 focus:outline-black active:bg-gray-100">
                  <div className="flex h-6 items-center justify-start space-x-2">
                    <ChevronRight className="h-5 w-5 shrink-0 text-gray-700 transition-all ease-in-out group-radix-state-open:rotate-90" />
                    <span className="text-sm font-medium text-gray-900">Expiration Date</span>
                    {expiresAt && new Date().getTime() > new Date(expiresAt).getTime() && (
                      <span className="bg-amber-500 px-2 py-0.5 text-xs uppercase text-white">Expired</span>
                    )}
                  </div>
                  {expiresAt && <CheckCircleFill className="h-6 w-6 text-black" />}
                </AccordionPrimitive.Trigger>
              </AccordionPrimitive.Header>
              <AccordionPrimitive.Content className="px-5 pb-3">
