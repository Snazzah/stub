  ...
  return (
    <>
      ...
      >
        ...
        <ChevronDown className={`h-5 w-5 text-gray-400 ${showFilters ? 'rotate-180 transform' : ''} transition-all duration-75`} />
      </button>
      <AnimatePresence>
        {showFilters && (
          <motion.div className="flex w-full flex-col space-y-2 border-t border-gray-200 py-5 sm:hidden" {...AnimationSettings}>
            <LinkSort />
            <StatusFilter />
            {slug && <UserFilter />}
          </motion.div>
